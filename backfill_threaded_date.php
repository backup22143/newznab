...
if ($ps->RunThreadCode()) 
{
	...
	echo "[Thread-{$thread}] Begining backfill processing for group {$group['name']}  $time\n";
	
	$param = $group['name'];
	
	$dir = dirname(__FILE__);
	$file = 'backfill_date.php';
	
	$output = shell_exec("php {$dir}/{$file} {$param}");
	//$output = shell_exec("/usr/bin/php -c /etc/php5/cli/php.ini {$dir}/{$file} {$param}");
	//$output = shell_exec("/usr/bin/php -c /etc/php5/cli/php.ini {$dir}/backfill.php {$param}");
